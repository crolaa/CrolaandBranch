...
hook.Add("Org Think", "Main", function(owner, org, timeValue)
	...
	module.eyes[2](owner, org, timeValue)
	if isPly then
...
concommand.Add("hg_organism_setvalue", function(ply, cmd, args)
	...
	local function applyOrganismValue(pl, key, val)
		if isbool(pl.organism[key]) then
			pl.organism[key] = tonumber(val) != 0
		else
			pl.organism[key] = tonumber(val)
		end

		if key == "eyeL" or key == "eyeR" then
			hg.organism.UpdateBlindness(pl.organism)
			pl.fullsend = true
			hg.send_organism(pl.organism, pl)
		end
	end

	if not args[3] then
		applyOrganismValue(ply, args[1], args[2])
	end

	if args[3] then
		for i, pl in pairs(player.GetListByName(args[3])) do
			applyOrganismValue(pl, args[1], args[2])
		end
	end
end)

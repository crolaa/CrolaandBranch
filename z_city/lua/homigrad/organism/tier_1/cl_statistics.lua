...
net.Receive("organism_send", function()
	...
	if add and org.owner.organism and org.owner.new_organism then
		...
		if org.owner == LocalPlayer() and org.owner.organism then
			for _, key in ipairs({"eyeL", "eyeR", "blindness"}) do
				if org[key] ~= nil then
					org.owner.organism[key] = org[key]
				end
			end

			org.owner.new_organism = table.Copy(org.owner.organism)
		end
	...
	if ply == LocalPlayer() and ply.organism then
		for _, key in ipairs({"eyeL", "eyeR", "blindness"}) do
			if org[key] ~= nil then
				ply.organism[key] = org[key]
			end
		end

		ply.new_organism = table.Copy(ply.organism)
	end

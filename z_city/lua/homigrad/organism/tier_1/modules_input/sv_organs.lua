...
local EYE_BLIND_THRESH = 0.55

local eyeMessages = {
	eyeL = {
		"Something hit my left eye...",
		"I can't see out of my left eye!",
		"My left eye— everything went dark on that side.",
	},
	eyeR = {
		"Something hit my right eye...",
		"I can't see out of my right eye!",
		"My right eye— everything went dark on that side.",
	},
}

local function damageEye(org, key, dmg, dmgInfo)
	local old = org[key] or 0
	local result = damageOrgan(org, dmg * 1.25, dmgInfo, key)
	local harmed = (org[key] or 0) - old

	if harmed <= 0 then return result end

	hg.AddHarmToAttacker(dmgInfo, harmed * 12, key .. " damage harm")

	org.painadd = org.painadd + dmg * 25
	org.shock = org.shock + dmg * 15
	org.disorientation = org.disorientation + dmg * 8
	org.fearadd = (org.fearadd or 0) + 0.5

	if org.isPly and not org.otrub and old < EYE_BLIND_THRESH and org[key] >= EYE_BLIND_THRESH then
		org.owner:Notify(table.Random(eyeMessages[key]), true, key, 0)
	end
...
input_list.eyeL = function(org, bone, dmg, dmgInfo)
	return damageEye(org, "eyeL", dmg, dmgInfo)
end

input_list.eyeR = function(org, bone, dmg, dmgInfo)
	return damageEye(org, "eyeR", dmg, dmgInfo)
end

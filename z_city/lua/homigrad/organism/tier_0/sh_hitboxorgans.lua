...
local eyeBoneForOrgan = {
	eyeL = "ValveBiped.Bip01_L_Eye",
	eyeR = "ValveBiped.Bip01_R_Eye",
}

local function shouldSkipHeadEyeFallback(ent, organName, nameBone)
	if nameBone ~= "ValveBiped.Bip01_Head1" then return false end
	local eyeBone = eyeBoneForOrgan[organName]
	return eyeBone and ent:LookupBone(eyeBone) ~= nil
end
...
function hg.organism.ShootMatrix(ent, organs)
	...
	for nameBone, organs in pairs(organs) do
		...
		for key, organ in pairs(organs) do
			...
			if shouldSkipHeadEyeFallback(ent, organName, nameBone) then continue end

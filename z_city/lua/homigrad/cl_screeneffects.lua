...
hook.Add("Post Post Processing", "ItHurts", function()
	...
	local organism = lply:Alive() and hg.organism.GetClientOrganism(lply) or (IsValid(spect) and hg.organism.GetClientOrganism(spect))
	if not organism then stopthings() return end
	if not organism.brain then stopthings() return end
	local org = organism

	if amtflashed >= 0.8 then
		hg.organism.DrawFlashEyeBlind()
	else
		hg.organism.DrawEyeBlindOverlay(org)
	end
...
local function removeflash()
	...
	if lply.blindflashes then
		for i, pt in ipairs(lply.blindflashes) do
			if IsValid(pt) then pt:Remove() end
		end
		lply.blindflashes = nil
	end
end

local function getBlindFlashAngles(view, eyesmode)
...
hook.Add("PreDrawOpaqueRenderables", "renderblindnessflash", function()
	...
	local flashModes = hg.organism.GetEyeBlindShaderModes(organism)
	if amtflashed >= 0.8 then
		flashModes = {2, 1}
	end

	if #flashModes == 0 then removeflash() return end

	local view = render.GetViewSetup(true)
	lply.blindflashes = lply.blindflashes or {}

	for i = 1, #flashModes do
		local pt = lply.blindflashes[i]
		if not IsValid(pt) then
			pt = ProjectedTexture()
			pt:SetTexture("effects/flashlight001")
			pt:SetEnableShadows(false)
			pt:SetConstantAttenuation(.1)
			lply.blindflashes[i] = pt
		end

		pt:SetFarZ(40)
		pt:SetFOV(160)
		pt:SetBrightness(1)
		pt:SetPos(view.origin)
		pt:SetAngles(getBlindFlashAngles(view, flashModes[i]))
		pt:Update()
	end

	for i = #flashModes + 1, #lply.blindflashes do
		if IsValid(lply.blindflashes[i]) then
			lply.blindflashes[i]:Remove()
		end
		lply.blindflashes[i] = nil
	end

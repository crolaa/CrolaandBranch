...
hook.Add("Org Think", "BodyTemperature", function(owner, org, timeValue) -- переделал систему температуры
	...
	if owner:Alive() and org.temperature > 40 then
		...
		if org.VomitCD < CurTime() then
			...
			owner:Notify(hg.get_phraselist(owner, "heatvomit"), 1, "phrase", 1, nil, Color(255,120,0, 255))
...
local function GetRandomRarity()
    ...
    for i = 1, #RarityWeights do
        total = total + RarityWeights[i].weight
    end

    local roll = math.Rand(0, total)

    for i = 1, #RarityWeights do
        local entry = RarityWeights[i]
...
local function GetRarityIndex(r)
    for i = 1, #RarityOrder do
        if RarityOrder[i] == r then
            return i
        end
    end
    return 1
...
-- =========================================================
-- CLASS → REPLACEMENT TABLES (NO MODELS)
-- =========================================================

local ClassReplacements = {

    ["prop_vehicle_jeep"] = {
        { class = "gtav_albany_esperanto", rarity = "Common" },
     { class = "gtav_albany_emperor_rusty", rarity = "Common" },
       { class = "gtav_albany_romero", rarity = "Common" },
       { class = "gtav_dundreary_regina", rarity = "Common" },
       { class = "brute_boxville_postop", rarity = "Legenday" },
       { class = "brute_boxville_mrtasty", rarity = "Legenday" },
        { class = "vapid_bobcat", rarity = "Epic" },
        { class = "vapid_interceptor", rarity = "Common" },

    },
    ["weapon_physcannon"] = {
        { class = "weapon_hg_gravgun_tpik", rarity = "Common" },
 		  { class = "weapon_physgun", rarity = "Abnormal" },
    },

  ["models/props_junk/bicycle01a.mdl"] = {
        { class = "bike", rarity = "Common" }
    },
  ["models/nseven/bicycle01a.mdl"] = {
        { class = "bike", rarity = "Common" }
    },
  ["models/props_bikes/bike_frame.mdl"] = {
        { class = "bike", rarity = "Common" }
    },
  ["models/props_c17/FurnitureCouch001a.mdl"] = {
        { class = "models/props_c17/FurnitureCouch001a.mdl", rarity = "Common" },
          { class = "sofa", rarity = "Legendary" },
  ...
  ["models/props_lab/monitor01a.mdl"] = {
        { class = "models/props_lab/monitor01a.mdl", rarity = "Common" },
          { class = "ent_templeos_terminal", rarity = "Legendary" },

    },
  ["models/props_silo/silo_launchroom_monitor.mdl"] = {
        { class = "models/props_silo/silo_launchroom_monitor.mdl", rarity = "Common" },
          { class = "ent_templeos_terminal", rarity = "Legendary" },

    },
  ["models/props_lab/monitor02.mdl"] = {
        { class = "models/props_lab/monitor02.mdl", rarity = "Common" },
          { class = "ent_templeos_terminal", rarity = "Legendary" },
...
local function SpawnReplacement(classOrModel, pos, ang,sourceClass)

    if not classOrModel then return end

    local ent

    -- If it's a model path, spawn a prop instead
    if string.find(classOrModel, ".mdl") then
        ent = ents.Create("prop_physics")
        if not IsValid(ent) then return end

        ent:SetModel(classOrModel)
    else
        ent = ents.Create(classOrModel)
        if not IsValid(ent) then return end
    end

    ent:SetPos(pos)
if sourceClass == "prop_vehicle_jeep" then
      ang:RotateAroundAxis(ang:Up(), 90)

  end
    ent:SetAngles(ang)
	ent.IsReplacement = true
    ent:Spawn()
    ent:Activate()
end

-- =========================================================
-- FALLBACK PICKER (RARITY SAFE)
-- =========================================================

local function PickWithFallback(list, rarity)
    local index = GetRarityIndex(rarity)

    for offset = 0, #RarityOrder do

        local down = index - offset
        if down >= 1 then
            local pool = {}

            for i = 1, #list do
                if list[i].rarity == RarityOrder[down] then
                    pool[#pool + 1] = list[i].class
                end
            end
        ...
        local up = index + offset
        if up <= #RarityOrder then
            local pool = {}

            for i = 1, #list do
                if list[i].rarity == RarityOrder[up] then
                    pool[#pool + 1] = list[i].class
                end
            end
...
local function HandleReplacement(ent)
    if not IsValid(ent) then return end

    local class = ent:GetClass()
    local model = ent:GetModel()

    local lootTable = ClassReplacements[class] or ClassReplacements[model]
    if not lootTable then return end

    local rarity = GetRandomRarity()
    local chosen = PickWithFallback(lootTable, rarity)
    if not chosen then return end

    SpawnReplacement(chosen, ent:GetPos(), ent:GetAngles(),class)
    SafeRemoveEntity(ent)
end
...
hook.Add("OnEntityCreated", "FunnySimfphys", function(ent)
    timer.Simple(0, function()
        if not IsValid(ent) then return end
    if ent.IsReplacement then return end
        timer.Simple(0.1, function()
            if not IsValid(ent) then return end
            HandleReplacement(ent)
        end)
    end)
